...
from torch.utils.checkpoint import context_fn_gen
...
#torch._dynamo.allow_non_fake_inputs=True
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, alibi):
        ...
        self.flash = (
            True  # hasattr(torch.nn.functional, "scaled_dot_product_attention")
        )
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.in_proj = torch.nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias)
        self.gelu = nn.GELU()
    ...
    def forward(self, x):
        if self.use_checkpointing:
            x = self.in_proj(x)
            # x = checkpoint.checkpoint(self.custom(self.in_proj), x, use_reentrant=False)
            x = checkpoint.checkpoint(self.custom(self.gelu),x,use_reentrant=False, context_fn=selective_checkpointing_context_fn)
            x = self.out_proj(x)
            # x = checkpoint.checkpoint(self.custom(self.out_proj), x, use_reentrant=False)
        else:
            x = self.in_proj(x)
            x = self.gelu(x)
            x = self.out_proj(x)
...
class Block(nn.Module):
    def __init__(self, config, alibi=None):
        super().__init__()
