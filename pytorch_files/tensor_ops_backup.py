...
def new_factory_rule(op_schema: OpSchema) -> OutputSharding:
    ...
    return OutputSharding(
        output_spec=DTensorSpec(
            ...
            placements=[Replicate()] * global_device_mesh.ndim,
            # tensor_meta=input.tensor_meta,
        )
    )

    """input = op_schema.args_schema[0]
    print(f"{input=}, {op_schema=}")
...
def new_loss_rule(op_schema: OpSchema) -> OutputSharding:
    global_device_mesh = get_global_device_mesh()
    input = op_schema.args_schema[0]
...
for op in new_loss_ops:
    register_prop_rule(op)(new_loss_rule)
