    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                h_avg = (
                    grad.mul(10.0 / lamda - 9.0)
                    .add(25.0 - 10.0 * lamda - 10 * (1 / lamda))
                    .add_(lamda_t * 10.0, state["g_1"])
                )
                ...
                if self.rectify:
                    ...
                    denom = exp_avg_sq.sqrt().add_(group["eps"])
                    step_size = math.sqrt(1.0 - beta2 ** state["step"]) / (
                ...
                alpha_t = 0.8 * (
                    1.0 + 1.0 / (0.1 * (torch.tensor(state["step"])).sqrt() + 3.8)
                )
